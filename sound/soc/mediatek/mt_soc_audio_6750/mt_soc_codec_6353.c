...
#ifdef __HCT_USING_EXTAMP_HP__
 #if __HCT_USING_EXTAMP_HP__^ defined(CONFIG_MTK_SPEAKER)
 #else
    #error "__HCT_USING_EXTAMP_HP__ complicated with CONFIG_MTK_SPEAKER, need check"
#endif
#endif
...
#define SPK_WARM_UP_TIME        (55)	/* unit is ms */
	...
	if (enable) {
		...
		AudDrv_GPIO_EXTAMP2_Select(false, extamp_sound_mode);
		...
		AudDrv_GPIO_EXTAMP2_Select(true, extamp_sound_mode);
#endif /*CONFIG_MTK_LEGACY*/
