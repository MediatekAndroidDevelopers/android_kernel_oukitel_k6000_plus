...
#ifdef __HCT_EXTAMP_GPIO_NUM__
	struct gpio_desc *extamp_gpio = NULL;
#endif
#ifdef __HCT_EXTAMP2_GPIO_NUM__
	struct gpio_desc *extamp2_gpio = NULL;
#endif
...
void AudDrv_GPIO_probe(void *dev)
{
	...
	int i = 0;
#ifdef __HCT_EXTAMP_GPIO_NUM__
		extamp_gpio = gpio_to_desc(__HCT_EXTAMP_GPIO_NUM__);
		gpiod_direction_output(extamp_gpio, 1);
#endif

#ifdef __HCT_EXTAMP_GPIO2_NUM__
		extamp2_gpio = gpio_to_desc(__HCT_EXTAMP_GPIO2_NUM__);
		gpiod_direction_output(extamp2_gpio, 1);
#endif
...
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1) {
		...
		for (i = 0; i < extamp_mode; i++) {
			gpiod_set_value(extamp_gpio, 0);
			udelay(1);
			gpiod_set_value(extamp_gpio, 1);
			udelay(1);
		}
...
int AudDrv_GPIO_EXTAMP2_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1) {
...
#ifdef __HCT_EXTAMP2_GPIO_NUM__
		for (i = 0; i < extamp_mode; i++) {
			gpiod_set_value(extamp2_gpio, 0);
			udelay(1);
			gpiod_set_value(extamp2_gpio, 1);
			udelay(1);
		}
		...
		if (aud_gpios[GPIO_EXTAMP2_HIGH].gpio_prepare) {
			for (i = 0; i < extamp_mode; i++) {
				retval = pinctrl_select_state(pinctrlaud,aud_gpios[GPIO_EXTAMP2_LOW].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXTAMP2_LOW] pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud,aud_gpios[GPIO_EXTAMP2_HIGH].gpioctrl);
				if (retval)
