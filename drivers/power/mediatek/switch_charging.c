...
static int ghct_temp_status = HCT_TEMP_STATE_UNKNOWN;
kal_bool hct_temp_error_recovery_chr_flag = KAL_TRUE;
...
void HCT_CHARGER_PLUGOUT_RESET(void)
{
	battery_log(BAT_LOG_CRTI,
			"[hctBATTERY] HCT_CHARGER_PLUGOUT_RESET  !!\n\r");

	ghct_temp_status =HCT_TEMP_STATE_UNKNOWN;

}


PMU_STATUS do_charger_state_machine(void)
{
	return PMU_STATUS_OK;
}


static void set_bat_protect_charging_current(void)
{
#ifdef CONFIG_USB_IF
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
...
////--> this is hct defined by hct custom		

	battery_log(BAT_LOG_CRTI,
			"[hctBATTERY] set_bat_protect_charging_current (%d) !!\n\r",
			BMT_status.temperature);

	if(BMT_status.charger_type == STANDARD_CHARGER)//(ghct_temp_status ==HCT_TEMP_STATE_UNKNOWN)  // this is first init
	{
		if (BMT_status.temperature > MAX_CHARGE_TEMPERATURE) 
		{
			ghct_temp_status = HCT_TEMP_ABOVE_POS_55;
			g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
		}else if(BMT_status.temperature >= TEMP_POS_50_THRESHOLD)
		{
			ghct_temp_status = HCT_TEMP_POS_50_TO_POS_55;
			g_temp_CC_value = AC_TEM_PROTECT_CHARGER_CURRENT;
		}
		else if(BMT_status.temperature >= TEMP_POS_0_THRESHOLD)
		{
			printk("hct add for set_bat_protect_charging_current = %d \n",g_temp_CC_value);
			ghct_temp_status = HCT_TEMP_POS_0_TO_POS_50;
			//g_temp_CC_value = AC_CHARGER_CURRENT;
		
		}
		else if(BMT_status.temperature >= HCT_TEMP_NEG_10_THRESHOLD){
			ghct_temp_status = HCT_TEMP_NEG_10_TO_POS_0;
			g_temp_CC_value = AC_LOW_TEM_PROTECT_CHARGER_CURRENT;
		}
		else
		{
			ghct_temp_status = HCT_TEMP_BELOW_NEG_10;
			g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
		}
		return;
      	}
...
#ifdef __HCT_G_TEMP_CC_VALUE__
				g_temp_CC_value = __HCT_G_TEMP_CC_VALUE__;
#else
...
#endif
#if __HCT_BATTERY_PROTECT_SUPPORT__
		set_bat_protect_charging_current();
#endif
...
static void mtk_select_cv(void)
{
	...
	if(batt_cust_data.high_battery_4400_voltage_support)
		cv_voltage = BATTERY_VOLT_04_400000_V;
	else if (batt_cust_data.high_battery_voltage_support)
