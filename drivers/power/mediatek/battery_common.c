...
#if 1 //def __HCT_BATTERY_CHARGER_CONTROL__
//#if __HCT_BATTERY_CHARGER_CONTROL__
static char battery_charger_status_value[5] = {};
 char battery_charger_status = 1;
static ssize_t show_Charger_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("show tp gesture status is %s \n",battery_charger_status_value);
	return scnprintf(buf, PAGE_SIZE, "%s\n", battery_charger_status_value);
}
static ssize_t store_Charger_status(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t size)
{
		if(!strncmp(buf, "on", 2))
		{
			sprintf(battery_charger_status_value,"on");
			battery_charger_status = 1;//status --- on
		}
		else
		{
			sprintf(battery_charger_status_value,"off");
			battery_charger_status = 0;//status --- off
		}
		return size;
}
static DEVICE_ATTR(ChargerStatus, 0664, show_Charger_status, store_Charger_status);
//#endif
#endif
/* ///////////////////////////////////////////////////////////////////////////////////////// */
