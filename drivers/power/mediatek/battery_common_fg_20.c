...
static struct work_struct otg_restart_work;
int flag_restart_otg = 0;
static struct work_struct otg_stop_work;
int flag_stop_otg = 0;
#endif
...
kal_bool upmu_is_chr_det(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[upmu_is_chr_det] otg tmp32=%d device=%d\n", tmp32, mt_usb_is_device());
#if defined(CONFIG_USB_XHCI_MTK)
	if((tmp32==0) && (mt_usb_is_device()==0))//otg line first, then U storge plug in,
	{
		if(flag_restart_otg == 0) //reset otg when U storge plug in the first time
		{
			schedule_work(&otg_restart_work);
			flag_restart_otg = 1;
			flag_stop_otg = 1;
		}
	}
	...
	if((tmp32==1) && (mt_usb_is_device()==1)) // otg plug out
	{
		if(flag_stop_otg == 1) //stop no use otg reset when otg plug out cause(reset otg work)
			schedule_work(&otg_stop_work);
		flag_stop_otg = 0;
	}
#endif
...
#if __HCT_T976_OZ115_CHARGE_UPDATE_AVGVBAT__ //0 open by huwenchang @20170717 // close by zhoush use this mthod shall result battery 0tranking fastly and freqence 
...
#else
#if defined(__HCT_BATTERY_PROTECT_SUPPORT__)
#if __HCT_BATTERY_PROTECT_SUPPORT__
	 if (do_charger_state_machine() == PMU_STATUS_FAIL) {
		 battery_log(BAT_LOG_CRTI, "[BATTERY] __HCT_BATTERY_PROTECT_SUPPORT__ : fail\n");
		 status = PMU_STATUS_FAIL;
	 }
#endif
#endif
...
#if  __HCT_BATTERY_PROTECT_SUPPORT__
extern void HCT_CHARGER_PLUGOUT_RESET(void);
#endif
...
extern void ncp1854_set_chg_en(unsigned long  val);
extern void ncp1854_set_otg_en(unsigned long  val);
extern void ncp1854_dump_register(void);
#endif
int g_flag_is_shutdown = 0;
#include <linux/gpio/consumer.h>
struct gpio_desc *otg_gpio = NULL;

static void otg_restart_work_callback(struct work_struct *work)
{
	battery_log(BAT_LOG_CRTI, "otg_restart_work_callback start\n");
	if (g_flag_is_shutdown)
	{
		battery_log(BAT_LOG_CRTI, "otg_restart_work_callback is shutdown, need return\n");
		return;
	}
	/*添加对OTG_EN的操作，重新打开vbus电压*/
#if defined(CONFIG_MTK_NCP1854_SUPPORT)
	otg_gpio = gpio_to_desc(43);
	gpiod_direction_output(otg_gpio, 1);

	ncp1854_set_otg_en(0);
	gpiod_set_value(otg_gpio, 0);
	msleep(20);

	ncp1854_set_otg_en(1);
	gpiod_set_value(otg_gpio, 1);

	if (g_flag_is_shutdown)
	{
		ncp1854_set_otg_en(0);
		gpiod_set_value(otg_gpio, 0);
		battery_log(BAT_LOG_CRTI, "otg_restart_work_callback is shutdown, must return\n");
		return;
	}
#endif
	battery_log(BAT_LOG_CRTI, "otg_restart_work_callback end\n");
}

static void otg_stop_work_callback(struct work_struct *work)
{
	battery_log(BAT_LOG_CRTI, "otg_stop_work_callback start\n");
#if defined(CONFIG_MTK_NCP1854_SUPPORT)
	otg_gpio = gpio_to_desc(43);
	gpiod_direction_output(otg_gpio, 1);
	ncp1854_set_otg_en(0);
	...
	msleep(20);                                                                                                                             
#endif
	battery_log(BAT_LOG_CRTI, "otg_stop_work_callback end\n");
}
#endif
...
#if (__HCT_BATTERY_VOLTAGE__==4400)
	batt_cust_data.high_battery_4400_voltage_support = 1;
#elif (__HCT_BATTERY_VOLTAGE__==4350)
...
static int battery_probe(struct platform_device *dev)
{
	...
	batt_init_cust_data();
#if defined(CONFIG_OZ115_SUPPORT)
	battery_log(BAT_LOG_CRTI, "[BAT_probe]  %d %d %d %d %d %d %d %d %d %d %d %d\n ",
			batt_cust_data.ta_ac_charging_current, batt_cust_data.ta_12v_support, 
			batt_cust_data.mtk_pump_express_plus_support, batt_cust_data.ta_ac_12v_input_current, batt_cust_data.ta_ac_9v_input_current,
			batt_cust_data.ta_ac_7v_input_current, batt_cust_data.ta_start_battery_soc, batt_cust_data.ta_stop_battery_soc, batt_cust_data.v_charger_max, batt_cust_data.ta_9v_support, batt_cust_data.ac_charger_input_current, batt_cust_data.ac_charger_current);

#ifdef CONFIG_OZ115_DUAL_SUPPORT
	#if __HCT_OZ115_CHARGER_4200MA__
	batt_cust_data.ta_ac_charging_current = 420000; //4.0 A
	#else
	batt_cust_data.ta_ac_charging_current = 400000; //4.0 A
	#endif
	#if  __HCT_OZ115_12V_SUPPORT__
	batt_cust_data.ta_12v_support = 1;
	#else	
	batt_cust_data.ta_12v_support = 0;  //0  改9V2A
	#endif
#else
	#ifdef __HCT_TA_AC_CHARGING_CURRENT__
	batt_cust_data.ta_ac_charging_current = __HCT_TA_AC_CHARGING_CURRENT__; //3A
	#else
	batt_cust_data.ta_ac_charging_current = 300000; //3A
	#endif
#endif
	batt_cust_data.mtk_pump_express_plus_support = 1;

	batt_cust_data.ta_ac_12v_input_current = 200000; //2A
	#ifdef __HCT_TA_AC_IN_CHARGING_CURRENT__
	batt_cust_data.ta_ac_9v_input_current = __HCT_TA_AC_IN_CHARGING_CURRENT__; //2A
	#else
	batt_cust_data.ta_ac_9v_input_current = 200000; //2A
	#endif
	batt_cust_data.ta_ac_7v_input_current = 200000; //2A
	batt_cust_data.v_charger_max = 16000; //16V
	batt_cust_data.ta_9v_support = 1;
	batt_cust_data.ac_charger_input_current = 200000; //2.0A
	batt_cust_data.ac_charger_current = 220000; //2.2A
	battery_charging_control(CHARGING_CMD_GET_PLATFORM_BOOT_MODE, &g_platform_boot_mode);
#if defined(CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT)
		batt_cust_data.ta_start_battery_soc = 2;
	else
#endif
		batt_cust_data.ta_start_battery_soc = 2;

	batt_cust_data.ta_stop_battery_soc = 110;
#endif
...
#if defined(CONFIG_USB_XHCI_MTK)
	INIT_WORK(&otg_restart_work, otg_restart_work_callback);
	INIT_WORK(&otg_stop_work, otg_stop_work_callback);
#endif
	/* LOG System Set */
	init_proc_log();
...
#endif
void hct_power_off_reset_vchr(void)
{
       kal_bool charging_enable = KAL_FALSE;
       printk("hct_power_off_reset_vchr -- start\n");
       ...
       printk("hct_power_off_reset_vchr -- end\n");
}
