...
static int pep_increase_ta_vchr(u32 vchr_target)
{
	...
	do {
		...
		retry_cnt++;
#if defined(CONFIG_MTK_KERNEL_POWER_OFF_CHARGING) //KERNEL_POWER_OFF_CHARGING_BOOT = 8
	} while (BMT_status.charger_exist && retry_cnt < (g_platform_boot_mode == 8 ? 3 : 6));
#else
...
int mtk_pep_start_algorithm(void)
{
	...
	if (batt_cust_data.ta_12v_support) {
		...
		pep_enable_vbus_ovp(false); //MAX 10.5
#endif
