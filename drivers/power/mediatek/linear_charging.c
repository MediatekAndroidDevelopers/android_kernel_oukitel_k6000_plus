...
static unsigned int charging_full_check(void)
{
	...
	if ((BMT_status.ICharging <= charging_full_current)&&(BMT_status.bat_vol > batt_cust_data.recharging_voltage)) {
		full_check_count++;
...
extern  char battery_charger_status;
//#endif
#endif
...
if (ta_check_ta_control == KAL_FALSE)
			...
			else if (batt_cust_data.high_battery_4400_voltage_support)
 ...
 #if 1 //def __HCT_BATTERY_CHARGER_CONTROL__
 //#if __HCT_BATTERY_CHARGER_CONTROL__
	if(battery_charger_status==1)
	{
		charging_enable= KAL_TRUE;
            battery_log(BAT_LOG_CRTI, "[BATTERY] pchr_charging_control_charging(), enable =%d \r\n",charging_enable);
	}
	else
	{
		charging_enable= KAL_FALSE;
