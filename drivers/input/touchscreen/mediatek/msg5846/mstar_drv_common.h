 ...
 * By default, this compile option is disabled.
 */
#if defined(__HCT_CONFIG_UPDATE_FIRMWARE_BY_SW_ID__)
	#if __HCT_CONFIG_UPDATE_FIRMWARE_BY_SW_ID__
 ...
 * By default, this compile option is disabled.
 */
#if defined(__HCT_CONFIG_ENABLE_CHARGER_DETECTION__)
	#if __HCT_CONFIG_ENABLE_CHARGER_DETECTION__
...
#define DBG(dev, fmt, arg...) DEBUG_LEVEL(0, dev, fmt, ##arg) 
/*
#define DEBUG_LEVEL(level, dev, fmt, arg...) do {\
	                                           if (level <= TOUCH_DRIVER_DEBUG_LOG_LEVEL)\
