...
#if FTS_GESTRUE_EN
static char ft_tpgesture_status_value[5] = {};
static char ft_tpgesture_status=0;
extern char ft_tpgesture_value[10];
#endif
...
#if 0//def CONFIG_MTK_I2C_EXTENSION
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		set_current_state(TASK_RUNNING);
			...
			TPD_DMESG("tpd fts_read_Gestruedata state=%d\n",state);
		     	if(state ==1)
		     	{
...
static int tpd_local_init(void)
{
 ...
 i2c_driver_add_err:   
    ...
    TPD_DEBUG("%s:ft_all_inone I2C Touchscreen init error\n",__FUNCTION__);
//    regulator_put(tpd->reg);
	err_get_reg:     
    return -1;
}
...
static void tpd_resume(struct device *h)
{
	...
	#if FTS_GESTRUE_EN
 		if ((g_call_state != CALL_ACTIVE)&&(ft_tpgesture_status))
		{
...
static ssize_t store_tpgesture_status_value(struct device* dev, struct device_attribute *attr, const char *buf, size_t count)
{
	 if(!strncmp(buf, "on", 2))
	 {
		 sprintf(ft_tpgesture_status_value,"on");
		 ft_tpgesture_status = 1;//status --- on
	 }
	 else
	 {
		 sprintf(ft_tpgesture_status_value,"off");
		 ft_tpgesture_status = 0;//status --- off
	 }
