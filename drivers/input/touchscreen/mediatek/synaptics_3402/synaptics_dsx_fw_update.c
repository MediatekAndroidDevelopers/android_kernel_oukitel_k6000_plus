...
static int fwu_start_reflash(void)
{
	...
	if (fwu->rmi4_data->sensor_sleep) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,"%s: Sensor sleeping\n",__func__);
		return -ENODEV;
	}
	...
	if (fwu->img.image == NULL) {
		...
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,"%s: Requesting firmware image %s\n",__func__, fwu->img.image_name);

		retval = request_firmware(&fw_entry, fwu->img.image_name,
				&fwu->rmi4_data->i2c_client->dev);
		if (retval != 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,"%s: Firmware image %s not available\n",	__func__, fwu->img.image_name);
			retval = -EINVAL;
			goto exit;
		}
#ifdef CONFIG_COMPAT
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,"%s: Firmware image size = %ld\n",__func__, fw_entry->size);
#else
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,"%s: Firmware image size = %d\n",__func__, fw_entry->size);
#endif
	...
	if (fwu->bl_version != fwu->img.bl_version) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,"%s: Bootloader version mismatch\n",__func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (f01_device_status.flash_prog) {
		dev_info(&fwu->rmi4_data->i2c_client->dev,"%s: In flash prog mode\n",__func__);
	...
	if (fwu->do_lockdown && (fwu->img.lockdown.data != NULL)) {
		...
		case V6:
			...
			if (retval < 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,"%s: Failed to do lockdown\n",__func__);
			}
			break;
		default:
			break;
...
#ifdef CONFIG_COMPAT
		dev_err(&rmi4_data->i2c_client->dev,"%s: Not enough space (%ld bytes) in buffer\n",__func__, count);
#else
		dev_err(&rmi4_data->i2c_client->dev,"%s: Not enough space (%d bytes) in buffer\n",__func__, count);
#endif
		return -EINVAL;
	}
