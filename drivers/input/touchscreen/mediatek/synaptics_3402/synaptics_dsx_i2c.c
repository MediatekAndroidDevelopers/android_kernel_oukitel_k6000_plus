...
#define SYNAPTICS_DRIVER_NAME  "synaptics_tpd"
...
#ifndef __HCT_PMIC_VTOUCH_SUPPORT__
#define __HCT_PMIC_VTOUCH_SUPPORT__    0
#endif

#ifndef __HCT_TP_POWER_AS_LCM_LDO_SUPPORT__
#define __HCT_TP_POWER_AS_LCM_LDO_SUPPORT__    0
#endif

#if __HCT_TP_POWER_AS_LCM_LDO_SUPPORT__
struct gpio_desc *lcm_powerdp_gpio = NULL;
struct gpio_desc *lcm_powerdm_gpio = NULL;
#endif
...
static irqreturn_t tpd_eint_handler(void);
...
{
	TPD_DEBUG_PRINT_INT;
	TPD_DMESG("tpd_eint_handler arthurwang get irq");
...
static int touch_event_handler(void *data)
{
	...
	TPD_DMESG("touch_event_handler arthurwang get irq\n");
	...
	do{
		...
		TPD_DMESG("touch_event_handler waiter arthurwang get irq\n");
		tpd_flag = 0;
		TPD_DEBUG_SET_TIME;
...
#endif
{
...
#if __HCT_TP_POWER_AS_LCM_LDO_SUPPORT__
		lcm_powerdp_gpio = gpio_to_desc(__HCT_GPIO_LCM_POWER_DP_PIN__);
                gpiod_direction_output(lcm_powerdp_gpio, 1);
		lcm_powerdm_gpio = gpio_to_desc(__HCT_GPIO_LCM_POWER_DM_PIN__);
                gpiod_direction_output(lcm_powerdm_gpio, 1);
#endif
	// gpio setting
	...
	// power up sequence
#if __HCT_PMIC_VTOUCH_SUPPORT__
...
//#if SYN_TPD_SUPPORT_I2C_DMA
	rmi4_data->i2c_client->dev.coherent_dma_mask = DMA_BIT_MASK(32);	
	gpDMABuf_va = (u8 *)dma_alloc_coherent(&rmi4_data->i2c_client->dev, 4096, &gpDMABuf_pa, GFP_KERNEL);	
	if(!gpDMABuf_va){
		printk("[Error] Allocate DMA I2C Buffer failed!\n");	
		return -1;
	}
...
//#endif
	...
	thread = kthread_run(touch_event_handler, rmi4_data, TPD_DEVICE);//"synaptics-tpd");
	if ( IS_ERR(thread) ) {
...
//	int ret;
	...
	tpd_halt = 1;
#if __HCT_TP_POWER_AS_LCM_LDO_SUPPORT__
	gpiod_set_value(lcm_powerdp_gpio, 0);
	gpiod_set_value(lcm_powerdm_gpio, 0);
...
static int tpd_local_init(void)
{
	...
	int retval;
#endif
	TPD_DEBUG("synaptics I2C Touchscreen Driver\n");
#if __HCT_PMIC_VTOUCH_SUPPORT__
	...
	retval = regulator_set_voltage(tpd->reg, 3300000, 3300000);	/* set 3.3v */
	if (retval)
     {   
		TPD_DEBUG("regulator_set_voltage() failed!\n");
            goto err_reg_set;
       }
#endif
	...
	return 0;
i2c_driver_add_err:   
    TPD_DEBUG("%s:synaptics I2C Touchscreen init error\n",__FUNCTION__);
#if __HCT_PMIC_VTOUCH_SUPPORT__
err_reg_set:
...
static struct tpd_driver_t synaptics_rmi4_driver = {
	.tpd_device_name = SYNAPTICS_DRIVER_NAME,
