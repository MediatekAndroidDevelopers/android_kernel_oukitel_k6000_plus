...
#define GYRO_TAG                  "[GYROSCOPEbmi160gyro] "
#define GYRO_FUN(f)               printk(GYRO_TAG"%s\n", __FUNCTION__)
#define GYRO_ERR(fmt, args...)    printk(GYRO_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
		...
		const struct i2c_device_id *id)
{
	...
	{
		GYRO_ERR("Bmi160_acc_i2c failed, gyro failed\n");
		goto exit;
	}
...
{
	const char *name = "mediatek,cust_bmi160_gyro";
