...
SP8407_VERSION sp8407version = SP8407R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = SP8407_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB0){
 				sp8407version = SP8407R1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, sp8407version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,sp8407version);	
				return ERROR_NONE;
				}
				//else if((read_cmos_sensor(0x302A)) == 0XB1){
 				//sp8407version = SP8407R2A;
				//LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, sp8407version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,sp8407version);	
				//return ERROR_NONE;
				//}
				//else{
				//LOG_INF("read sp8407 R1A R2A bate fail\n");	  
				//return ERROR_SENSOR_CONNECT_FAIL;
				//}
			}	
			LOG_INF("i2c write id: 0x%x,Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
