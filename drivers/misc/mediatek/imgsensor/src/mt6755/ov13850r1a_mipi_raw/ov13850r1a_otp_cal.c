...
#include "ov13850r1amipiraw_Sensor.h"
#include "ov13850r1a_otp.h"

#if 1
extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

//#define OV13850_R1A_write_i2c(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV13850MIPI_WRITE_ID)
#define PFX "OV13850_R1A_OTP"
...
kal_uint16 OV13850_R1A_read_i2c(kal_uint32 addr)
{
...
kal_uint16 OV13850_R1A_write_i2c(kal_uint32 addr, kal_uint16 para)
{
...
void otp_cali_r1a(unsigned char writeid)
{
	struct otp_struct current_otp;
	OV13850MIPI_WRITE_ID = writeid;
	memset(&current_otp, 0, sizeof(struct otp_struct));
	read_otp_r1a(&current_otp);
	apply_otp_13850r1a(&current_otp);
}

int Decode_13850R1A(unsigned char*pInBuf, unsigned char* pOutBuf)
{
	if(pInBuf != NULL)
	{
		LumaDecoder_r1a(pInBuf, pOutBuf);
		ColorDecoder_r1a((pInBuf+86), (pOutBuf+120));
		ColorDecoder_r1a((pInBuf+136), (pOutBuf+240));
		LOG_INF(" OTP OK \n");
		return 1;
	}
	{
...
int read_otp_r1a(struct otp_struct *otp_ptr)
{
	...
	//set 0x5002[1] to "0"
	kal_uint16 temp1=0;
	temp1 = OV13850_R1A_read_i2c(0x5002);
	OV13850_R1A_write_i2c(0x5002, (0x00 & 0x02) | (temp1 & (~0x02)));
	// read OTP into buffer
	OV13850_R1A_write_i2c(0x3d84, 0xC0);
	OV13850_R1A_write_i2c(0x3d88, 0x72); // OTP start address
	OV13850_R1A_write_i2c(0x3d89, 0x20);
	OV13850_R1A_write_i2c(0x3d8A, 0x73); // OTP end address
	OV13850_R1A_write_i2c(0x3d8B, 0x04);
	OV13850_R1A_write_i2c(0x3d81, 0x01); // load otp into buffer
	Delay(10);
	// OTP base information and WB calibration data
	otp_flag = OV13850_R1A_read_i2c(0x7220);
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		addr = 0x7226; // base address of info group 2
	}
	else if((otp_flag & 0x0c) == 0x04) {
		addr = 0x722b; // base address of info group 3
	}
	if(addr != 0) {
		(*otp_ptr).flag = 0x80; // valid info and AWB in OTP
		(*otp_ptr).module_integrator_id = OV13850_R1A_read_i2c(addr);
		(*otp_ptr).lens_id = OV13850_R1A_read_i2c( addr + 1);
		(*otp_ptr).production_year = OV13850_R1A_read_i2c( addr + 2);
		(*otp_ptr).production_month = OV13850_R1A_read_i2c( addr + 3);
		(*otp_ptr).production_day = OV13850_R1A_read_i2c(addr + 4);
	}
	else {
	...
	otp_flag = OV13850_R1A_read_i2c(0x7230);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7231; // base address of VCM Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7236; // base address of VCM Calibration group 2
	}
	else if((otp_flag & 0x0c) == 0x04){
		addr = 0x723b;
	}
	if(addr != 0){
		(*otp_ptr).flag |= 0x40;
		temp = OV13850_R1A_read_i2c(addr + 4);
	    (*otp_ptr).rg_ratio = (OV13850_R1A_read_i2c(addr)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13850_R1A_read_i2c(addr + 1)<<2) + ((temp>>4) & 0x03);	
	}
	else{
		(*otp_ptr).rg_ratio = 0;
		(*otp_ptr).bg_ratio = 0;
	...
	// OTP VCM Calibration
	otp_flag = OV13850_R1A_read_i2c(0x7340);
	LOG_INF(" VCM calibration data : %x \n", otp_flag);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7241; // base address of VCM Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7244; // base address of VCM Calibration group 2
	}
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		temp = OV13850_R1A_read_i2c(addr + 2);
		(* otp_ptr).VCM_start = (OV13850_R1A_read_i2c(addr)<<2) | ((temp>>6) & 0x03);
		(* otp_ptr).VCM_end = (OV13850_R1A_read_i2c(addr + 1) << 2) | ((temp>>4) & 0x03);
	...
	otp_flag = OV13850_R1A_read_i2c(0x724a);
	LOG_INF(" Lenc calibration data : %x \n", otp_flag);
	addr = 0;
	//int checksumLSC = 0, checksumOTP = 0, checksumTotal = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x724b; // base address of Lenc Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7289; // base address of Lenc Calibration group 2
	}
	//
	LOG_INF(" Lenc calibration addr : %x \n", addr);
if(addr != 0) {
		for(i=0;i<62;i++) {
			(* otp_ptr).lenc[i]= OV13850_R1A_read_i2c(addr + i);
		...
		if(Decode_13850R1A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<62;i++) 
			{
		...
		(* otp_ptr).checksumLSC=OV13850_R1A_read_i2c(addr + 186);
		(* otp_ptr).checksumOTP=OV13850_R1A_read_i2c(addr + 187);
		(* otp_ptr).checksumTotal=OV13850_R1A_read_i2c(addr + 188);
	...
	for(i=0x7220;i<=0x7304;i++) 
	{
		OV13850_R1A_write_i2c(i,0); // clear OTP buffer, recommended use continuous write to accelarate
	}

	//set 0x5002[1] to "1"
	temp1 = OV13850_R1A_read_i2c(0x5002);
	OV13850_R1A_write_i2c(0x5002, (0x02 & 0x02) | (temp1 & (~0x02)));
...
// bit[5]: 0 no otp vcm, 1 valid otp vcm
// bit[4]: 0 no otp lenc, 1 valid otp lenc
int apply_otp_13850r1a(struct otp_struct *otp_ptr)
{
	int rg=0;
	int bg=0;
	int R_gain=0;
	...
	int i=0;
	unsigned char lenc_out[360];
	LOG_INF(" apply_otp_13850r1a (*otp_ptr).flag : %x \n", (*otp_ptr).flag);
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		if (R_gain>0x400) {
			OV13850_R1A_write_i2c(0x5056, R_gain>>8);
			OV13850_R1A_write_i2c(0x5057, R_gain & 0x00ff);
		}

		if (G_gain>0x400) {
			OV13850_R1A_write_i2c(0x5058, G_gain>>8);
			OV13850_R1A_write_i2c(0x5059, G_gain & 0x00ff);
		}

		if (B_gain>0x400) {
			OV13850_R1A_write_i2c(0x505A, B_gain>>8);
			OV13850_R1A_write_i2c(0x505B, B_gain & 0x00ff);
		}
	}
	...
	if ((*otp_ptr).flag & 0x10) {
		...
		temp = OV13850_R1A_read_i2c(0x5000);
		temp = 0x01 | temp;
		OV13850_R1A_write_i2c(0x5000, temp);
		...
		/***For function Decode_13850R1A(unsigned char*pInBuf, unsigned char* pOutBuf),please refer to lc42.h***/
		if(Decode_13850R1A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360 ;i++) {
				LOG_INF(" apply OTP lenc_out[%d]:%x \n", i, lenc_out[i]);
				OV13850_R1A_write_i2c(0x5200 + i, lenc_out[i]);
			}
		}
	}
return 1;
...
void LumaDecoder_r1a(uint8_t *pData, uint8_t *pPara)
{
...
//
void ColorDecoder_r1a(uint8_t *pData, uint8_t *pPara)
{
