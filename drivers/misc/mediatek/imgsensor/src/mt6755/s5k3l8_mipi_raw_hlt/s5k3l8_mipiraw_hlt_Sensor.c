...
#include "s5k3l8_mipiraw_hlt_Sensor.h"
...
#define LOG_1 LOG_INF("S5K3L8_HLT,MIPI 4LANE\n")
#define SENSORDB LOG_INF
/****************************   Modify end    *******************************************/

#define S5K3L8_HLT_USE_AWB_OTP

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_HLT_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
struct S5K3L8_HLT_MIPI_otp_struct{
...
static kal_uint16 eeprom_read(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
    char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 2, EEPROM_WRITE_ID);
...
void S5K3L8_HLT_MIPI_read_otp_wb(struct S5K3L8_HLT_MIPI_otp_struct *otp)
{
...
void S5K3L8_HLT_MIPI_write_otp_wb(struct S5K3L8_HLT_MIPI_otp_struct *otp)
{
  ...
  LOG_INF("S5K3L8_HLT_OTP:Gr_gain=0x%x\n",Gr_gain);
	LOG_INF("S5K3L8_HLT_OTP:R_gain=0x%x\n",R_gain);
	LOG_INF("S5K3L8_HLT_OTP:B_gain=0x%x\n",B_gain);
	LOG_INF("S5K3L8_HLT_OTP:Gb_gain=0x%x\n",Gb_gain);
	LOG_INF("S5K3L8_HLT_OTP:End.\n");
}

void S5K3L8_HLT_MIPI_update_wb_register_from_otp(void)
{
   struct S5K3L8_HLT_MIPI_otp_struct current_otp;  
   S5K3L8_HLT_MIPI_read_otp_wb(&current_otp);
   S5K3L8_HLT_MIPI_write_otp_wb(&current_otp);
}
...
//ihdr_write_shutter_gain not support for s5k3l8_hlt
...
//$MV1[MCLK:24,Width:2104,Height:1560,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
  ...
  #ifdef S5K3L8_HLT_USE_AWB_OTP
	S5K3L8_HLT_MIPI_update_wb_register_from_otp();
	#endif
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			#ifdef FPTPDAFSUPPORT
			S5K3L8_HLT_read_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			#endif
			break;	
        /******************** PDAF END   <<< *********/
        default:
...
UINT32 S5K3L8_MIPI_RAW_HLT_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L8_MIPI_RAW_HLT_SensorInit	*/
