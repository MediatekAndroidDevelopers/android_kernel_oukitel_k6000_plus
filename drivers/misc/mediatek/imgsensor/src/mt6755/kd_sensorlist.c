...
extern SENSOR_FUNCTION_STRUCT_1	SensorFuncSP0A20YUV_1;
//static SENSOR_FUNCTION_STRUCT_1 *g_pSensorFunc_1 = &SensorFuncSP0A20YUV_1;
extern SENSOR_FUNCTION_STRUCT_1	SensorFuncGC0310YUV_1;
//static SENSOR_FUNCTION_STRUCT_1 *g_pSensorFunc_2 = &SensorFuncGC0310YUV_1;
static SENSOR_FUNCTION_STRUCT_1 *g_pSensorFunc_1[2] = {&SensorFuncGC0310YUV_1, &SensorFuncSP0A20YUV_1};
static char buf_yuv_name[32];
static MINT8 j = 0;
#endif
...
#if  __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
int GC0310_iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    int  i4RetValue = 0;
    if (gI2CBusNum_main2 == SUPPORT_I2C_BUS_NUM1) {
    spin_lock(&kdsensor_drv_lock);
    g_pstI2Cclient->addr = (i2cId >> 1);
    g_pstI2Cclient->ext_flag = (g_pstI2Cclient->ext_flag)&(~I2C_DMA_FLAG);
...
int GC0310_iWriteRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u16 i2cId)
{
    int  i4RetValue = 0;
    int retry = 3;
...
#if  __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
{
	if (i==0)//when it is main	poweron main2
		{
	/* wait for power stable */
	strcpy(buf_yuv_name, "gc0310mipiyuv");
	mDELAY(20);
	KD_IMGSENSOR_PROFILE("kdModulePowerOn");
	
	/* turn on power */
	ret_main2 = kdCISModulePowerOn(DUAL_CAMERA_MAIN_2_SENSOR, (char *)buf_yuv_name, true, CAMERA_HW_DRVNAME1);
	#if 0
	if (ERROR_NONE != ret_main2) {
	PK_ERR("[%s]", __func__);
	return ret_main2;
	}
	#endif
	ret_main2=g_pSensorFunc_1[j]->SensorGetID();
	printk("gc0310 SensorGetID ret_main2:%d\n",ret_main2);
	if (ERROR_NONE != ret_main2) {
		j=1;
		strcpy(buf_yuv_name, "sp0a20mipiyuv");
		ret_main2 = kdCISModulePowerOn(DUAL_CAMERA_MAIN_2_SENSOR, (char *)buf_yuv_name, true, CAMERA_HW_DRVNAME1);
		#if 0
		if (ERROR_NONE != ret_main2) {
		PK_ERR("[%s]", __func__);
		...
		#endif
		ret_main2=g_pSensorFunc_1[j]->SensorGetID();
		printk("sp0a20 SensorGetID ret_main2:%d\n",ret_main2);
	    }
	}
}
#endif
        ...
        if (DUAL_CAMERA_MAIN_2_SENSOR == g_invokeSocketIdx[i]) {
            ...
            spin_unlock(&kdsensor_drv_lock);
            //printk("kd_MultiSensorOpen: switch I2C BUS%d\n", gI2CBusNum);
        }
...
#if  __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
		if (i==0)//when it is main,open main2
			{
			ret_main2=g_pSensorFunc_1[j]->SensorOpen();
			printk("zengyuan ret_main2:%d\n",ret_main2);
			}
{
...
#if __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
{
	//char buf_yuv_name[32];
	//strcpy(buf_yuv_name, "sp0a20mipiraw");

			if (i==0)//when it is main,poweroff main2
				{
					kdCISModulePowerOn(DUAL_CAMERA_MAIN_2_SENSOR, (char *)buf_yuv_name, false, CAMERA_HW_DRVNAME1);
				}
}
#endif
        PK_ERR("SensorOpen");
        ...
        return ret;
		...
		#if  __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
			{
				...
				if (i==0)//when it is main	poweroff main2
					{
						/* turn on power */
						ret = kdCISModulePowerOn(DUAL_CAMERA_MAIN_2_SENSOR, (char *)buf_yuv_name, false, CAMERA_HW_DRVNAME1);
						if (ERROR_NONE != ret) {
							PK_ERR("[%s]", __func__);
							return ret;
						}
...
static int yuvcamera_create_attr(struct device_driver *driver)
{
    int idx, err = 0;
...
static int yuvcamera_delete_attr(struct device_driver *driver)
{
    int idx , err = 0;
    int num = (int)(sizeof(yuvcamera_attr_list)/sizeof(yuvcamera_attr_list[0]));
...
#if  __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
	ret = yuvcamera_create_attr(&g_stCAMERA_HW_Driver.driver);
	if (ret)
	PK_ERR("device_create_file vibr_on fail!\n");
#endif
...
static void __exit CAMERA_HW_i2C_exit(void)
{
	...
	yuvcamera_delete_attr(&g_stCAMERA_HW_Driver.driver);
#endif
