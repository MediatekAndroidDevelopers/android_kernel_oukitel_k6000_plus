...
//#define LOG_1 printk("IMX278,MIPI 4LANE\n")
//#define LOG_2 printk("preview 2100*1560@30fps,864Mbps/lane; video 4208*3120@30fps,864Mbps/lane; capture 13M@30fps,864Mbps/lane\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = { 
    ...
    .pre = {
		...
		.grabwindow_height = 1558,//1552,		//record different mode's height of grabwindow
...
{{ 4208, 3120, 0000, 0000, 4208, 3120, 2100, 1558, 0002, 0000, 2100, 1558, 0000, 0000, 2100, 1558}, // Preview 2112*1558
...
{
	printk("Enter sensor_init,FOR TEST\n");
...
static void preview_setting(void)   //PreviewSetting
{
  printk("Enter sensor_preview\n");
    ...
    write_cmos_sensor(0x0342 , 0x13);//line length  4976
    write_cmos_sensor(0x0343 , 0x70);
	
    write_cmos_sensor(0x0340 , 0x0C);//frame length 3200
    ...
    write_cmos_sensor(0x0348 , 0x10);//4207
    write_cmos_sensor(0x0349 , 0x6F);
	
    write_cmos_sensor(0x034A , 0x0C);//3119
	...
	//output
    write_cmos_sensor(0x034C , 0x08);//2100
    write_cmos_sensor(0x034D , 0x34);
	
    write_cmos_sensor(0x034E , 0x06);//1558
    write_cmos_sensor(0x034F , 0x16);
   ...
   //kal_uint32 p1,p2;
    ...
    //printk("preview setting 0x0100 = %d\n",p1);
...
if(currefps==300)
    ...
    write_cmos_sensor(0x0341, 0x94);// 94
    //ROI setting
    write_cmos_sensor(0x0344, 0x00);
    write_cmos_sensor(0x0345, 0x00);  
    write_cmos_sensor(0x0346, 0x00); 
   ...
   //Integration Time Setting
   write_cmos_sensor(0x0202, 0x0C);
   write_cmos_sensor(0x0203, 0x82);//8A
   //Gain setting
...
static void normal_video_setting(kal_uint16 currefps)    // VideoFullSizeSetting
{
	 ...
	 preview_setting();
   #if 0
   printk("Enter normal_video_setting 4k2k\n");
...
static void setIso(SET_SENSOR_ISO* para)
{
    ...
    write_cmos_sensor(0x3011, 0xff);
    printk("rgbw setIso, iso=%d mode=%d, reg=%d\n", iso, sensorMode, val);
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_SENSOR_OTP_AWB_CMD:
            printk("Update sensor awb from otp :%d\n", (BOOL)*feature_data);
            spin_lock(&imgsensor_drv_lock);
        ...
        case SENSOR_FEATURE_SET_SENSOR_OTP_LSC_CMD:
            printk("Update sensor lsc from otp :%d\n", (BOOL)*feature_data);
            spin_lock(&imgsensor_drv_lock);
