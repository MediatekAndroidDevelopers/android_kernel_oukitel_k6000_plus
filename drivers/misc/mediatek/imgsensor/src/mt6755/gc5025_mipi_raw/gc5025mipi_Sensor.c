...
static void gc5025_gcore_read_otp_info(void)
{
	...
	/*DD check sum*/
	gc5025_select_page_otp(otp_page1);
	ddchecksum = gc5025_read_otp(0x75);
	check = total_number;
...
#define ANALOG_GAIN_3 130  // 2.028x
#define ANALOG_GAIN_4 188  // 2.935x
#define ANALOG_GAIN_5 264  // 4.12x
//#define ANALOG_GAIN_6 358  //5.6X     5.546x_juan0607 5.83x
//#define ANALOG_GAIN_7 512  //8X      7.82x_juan0607 8.325x
//#define ANALOG_GAIN_8 716  //11.2X    10.95_juan0607 11.60x
//#define ANALOG_GAIN_9 1028  // 16.067x
//#define ANALOG_GAIN_10 1437  // 22.459x
//#define ANALOG_GAIN_11 2092  // 32.695x
//#define ANALOG_GAIN_12 2620  // 40.945x
//#define ANALOG_GAIN_12 3805  // 59.463x
...
{	
	...
	if((iReg >= 0x40)&&(iReg < 0x100)) 	
	{
		if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	    {		
			//analog gain
			write_cmos_sensor(0xfe,  0x00);
			write_cmos_sensor(0x34,  0x52);
				//write_cmos_sensor(0xcd,  0xaf);
	    ...
	    {		
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
			LOG_INF("GC5025MIPI analogic gain 1.469x , GC5025MIPI add pregain = %d\n",temp);
	    }
	}
	else 
	{
