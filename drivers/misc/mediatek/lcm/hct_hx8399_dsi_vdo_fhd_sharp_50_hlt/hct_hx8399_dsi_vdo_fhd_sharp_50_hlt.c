...
static unsigned int lcm_compare_id(void)
{
 ...
 #if defined(BUILD_LK)
         printf("hct_8399-hlt adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
  #else
          printk("hct_8399-hlt adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
  #endif
  rawdata=0;
...
LCM_DRIVER hct_hx8399_dsi_vdo_fhd_sharp_50_hlt= 
{
    .name			= "hct_hx8399_dsi_vdo_fhd_sharp_50_hlt",
