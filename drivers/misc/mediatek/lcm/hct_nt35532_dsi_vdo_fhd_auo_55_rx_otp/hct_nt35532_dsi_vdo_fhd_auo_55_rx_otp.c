...
#if defined(BUILD_LK)
	printf("hct_nt35532_dsi_vdo_fhd_auo_55_rx_otp adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#else
	printk("hct_nt35532_dsi_vdo_fhd_auo_55_rx_otp adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#endif

	if((rawdata > 0x100) && (rawdata < 0x900) && (ret == 0))
	{	 
...
LCM_DRIVER hct_nt35532_dsi_vdo_fhd_auo_55_rx_otp =
{
    .name            = "hct_nt35532_dsi_vdo_fhd_auo_55_rx_otp",
