...
static unsigned int lcm_compare_id(void)
{
...
#if defined(BUILD_LK)
	printf("hct_nt35532_dsi_vdo_fhd_auo_55_hz adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#else
	printk("hct_nt35532_dsi_vdo_fhd_auo_55_hz adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#endif

	if(rawdata > 0x100 && rawdata <0xA00 && (ret==0))
	{	 
    ...
    dsi_set_cmdq(array, 1, 1);
        ...
        #ifdef BUILD_LK
            printf("%s, hct_nt35532_dsi_vdo_fhd_auo_55_hz debug: id = 0x%08x\n", __func__, id);
        #else
            printk("%s, hct_nt35532_dsi_vdo_fhd_auo_55_hz debug: id = 0x%08x\n", __func__, id);
        #endif
...
LCM_DRIVER hct_nt35532_dsi_vdo_fhd_auo_55_hz =
{
    .name            = "hct_nt35532_dsi_vdo_fhd_auo_55_hz",
