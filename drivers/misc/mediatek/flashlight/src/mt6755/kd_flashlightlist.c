...
#if defined(__HCT_RM_LOWBAT_FLASH_PROTECT__)
#else
#define __HCT_RM_LOWBAT_FLASH_PROTECT__ 0
#endif
/******************************************************************************
 * Definition
******************************************************************************/
...
struct pinctrl_state *flashlight_sub_as_pwm = NULL;
struct pinctrl_state *flashlight_sub_pwm_l = NULL;
struct pinctrl_state *flashlight_sub_pwm_h = NULL;

static struct device_node *flashlight_device_node = NULL;
...
#endif
	/*flashlight as pwm initialization */
	flashlight_sub_as_pwm = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_sub_as_pwm");
	if (IS_ERR(flashlight_sub_as_pwm)) {
		ret = PTR_ERR(flashlight_sub_as_pwm);
		pr_debug("%s : pinctrl err, flashlight_as_pwm\n", __func__);
	}

	/*flashlight pwm Pin initialization */
	flashlight_sub_pwm_l = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_sub_pwm0");
	if (IS_ERR(flashlight_sub_pwm_l)) {
		ret = PTR_ERR(flashlight_sub_pwm_l);
		pr_debug("%s : pinctrl err, flashlight_pwm_l\n", __func__);
	}

	/*flashlight pwm Pin initialization */
	flashlight_sub_pwm_h = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_sub_pwm1");
	if (IS_ERR(flashlight_sub_pwm_h)) {
		ret = PTR_ERR(flashlight_sub_pwm_h);
		pr_debug("%s : pinctrl err, flashlight_pwm_h\n", __func__);
	}
...
void flashlight_gpio_as_pwm(int pin, int config)
{
	...
	{   
		if(IS_ERR(flashlight_as_pwm)){
			pr_err( "err: flashlight_as_pwm is error!!!");
			return;
		}else
			pinctrl_select_state(flashlightpinsctrl, flashlight_as_pwm);
	}else if(FL_SUB_PWM_PIN == pin){
		if(IS_ERR(flashlight_sub_as_pwm)){
...
void flashlight_gpio_output(int pin, int level)
{
	...
	case FL_SUB_EN_PIN: 	
     ...
     case FL_SUB_PWM_PIN:     
          if(IS_ERR(flashlight_sub_pwm_h)||IS_ERR(flashlight_sub_pwm_l))
          {
               pr_err("err: flashlight_sub_pwm_h or flashlight_sub_pwm_l is error!!!");
               return;
          }   
          else
           break;
	...
	case FL_SUB_PWM_PIN: 	pinctrl_select_state(flashlightpinsctrl, level ? flashlight_sub_pwm_h : flashlight_sub_pwm_l); break; 
	default: ;
	}
}
#endif

gpio_pwm_t gpio_pwm_tab[] ={
	{3,PWM_A},
	{4,PWM_B},
	{5,PWM_C},
	{8,PWM_A},
...
int flashlight_gpio_as_get_pwm_no(int gpio_no){
	int i;
	int pwm_no = -1;
	for(i=0; i < ARRAY_SIZE(gpio_pwm_tab); i++){
		if(gpio_no == gpio_pwm_tab[i].gpio_no){
			pwm_no = gpio_pwm_tab[i].pwm_no;
			return pwm_no;
		}
	}
	logI("error:gpio:%d is not support pwm mode!\n",gpio_no);
	return -1;
}

int flashlight_gpio_pwm_start(int pwm_no,int duty){
	int ret = 0;
...
//	    .PWM_MODE_OLD_REGS.THRESH = 5,
	...
	if(pwm_no < PWM_MIN || pwm_no > PWM_MAX){
		logI("error: pwm_no set failed,pwm_no:%d\n",pwm_no);
		return -1;
	}
	if(duty < 0 || duty > 10){
		logI("error: duty set failed,duty:%d\n",duty);
		return -1;
	}
	pwm_config.pwm_no = pwm_no;
	pwm_config.PWM_MODE_OLD_REGS.THRESH = duty;	

	ret = pwm_set_spec_config(&pwm_config);
	return ret;
}

inline int flashlight_gpio_pwm_end(int pwm_no){
	if(pwm_no < PWM_MIN || pwm_no > PWM_MAX){
		logI("error: pwm_no set failed,pwm_no:%d\n",pwm_no);
		return -1;
	}
...
#if __HCT_RM_LOWBAT_FLASH_PROTECT__
#else
...
#if 1 //def MTK_SECURITY_SW_SUPPORT //chenyichong add for hct efuse.
extern char *saved_command_line;
static int get_hct_pl_seclib_sbc_enabled_flag(void)
{
	char *ptr;
	int ret=0;
	ptr=strstr(saved_command_line,"hct_pl_seclib_sbc_enabled=");
	if(ptr==NULL)
	return 0;
	ptr+=strlen("hct_pl_seclib_sbc_enabled=");
	ret=simple_strtol(ptr,NULL,10);
	return ret;
} 

static int get_hct_lk_efuse_sbc_enabled_flag(void)
{
	char *ptr;
	int ret=0;
	ptr=strstr(saved_command_line,"hct_lk_efuse_sbc_enabled=");
	if(ptr==NULL)
	return 0;
	ptr+=strlen("hct_lk_efuse_sbc_enabled=");
	ret=simple_strtol(ptr,NULL,10);
	return ret;
} 


static ssize_t show_hct_pl_efuse_value(struct device_driver *ddri, char *buf)
{
	u8 ret = get_hct_pl_seclib_sbc_enabled_flag();

	printk("show_hct_pl_efuse_value: status is %d.\n",ret);
	return scnprintf(buf, PAGE_SIZE, "%d\n", ret);
}

static ssize_t show_hct_lk_efuse_value(struct device_driver *ddri, char *buf)
	{
		u8 ret = get_hct_lk_efuse_sbc_enabled_flag();
	
		printk("show_hct_lk_efuse_value: status is %d.\n",ret);
		return scnprintf(buf, PAGE_SIZE, "%d\n", ret);
	}

static DRIVER_ATTR(hct_pl_efuse,     /*S_IWUSR | S_IRUGO*/0664, show_hct_pl_efuse_value, NULL);
static DRIVER_ATTR(hct_lk_efuse,     /*S_IWUSR | S_IRUGO*/0664, show_hct_lk_efuse_value, NULL);
#endif
...
#if 1 //def MTK_SECURITY_SW_SUPPORT //chenyichong add for hct efuse.
    &driver_attr_hct_pl_efuse,
    &driver_attr_hct_lk_efuse,
#endif
};
...
#define FLASHLIGHT_MAJOR 242
	...
	flashlight_device_node = of_find_matching_node(flashlight_device_node, FLASHLIGHT_of_match);
	...
	if (flashlight_list.fl_init){
		flashlight_list.fl_init(dev,flashlight_device_node);
	}
#ifdef CONFIG_HCT_FLASHLIGHT_SUB_SUPPORT
#ifdef CONFIG_HCT_DEVICE_INFO_SUPPORT
	...
	if (flashlight_list_sub.fl_init){
		flashlight_list_sub.fl_init(dev,flashlight_device_node);
	}
#endif
