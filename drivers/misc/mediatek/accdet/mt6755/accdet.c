...
extern int typec_irqnum;
int typec_gpoi_status;
int typec_recheck_flag=1;
struct work_struct accdet_eint_work;
...
irqreturn_t accdet_eint_func(int irq, void *data)
{
	int ret = 0;
...
#endif
#ifndef __HCT_ACCDET_EARPHONE_ONLY_MD_KEY_SUPPORT__
#define __HCT_ACCDET_EARPHONE_ONLY_MD_KEY_SUPPORT__ 0
#endif
static void send_key_event(int keycode, int flag)
{
	switch (keycode) {
#if !__HCT_ACCDET_EARPHONE_ONLY_MD_KEY_SUPPORT__
	case DW_KEY:
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	if (1 == eint_accdet_sync_flag)
	{
		ACCDET_DEBUG("[Accdet] Headset has plugged in , and type=%d, rechek=%d\n",cable_type, typec_recheck_flag);
		...
		{  // in this case we need reset accdet and recheckit
				...
				typec_gpoi_status=gpio_get_value(__HCT_TYPEC_ACCDET_MIC_SWITCH_GPIO_NUM__);
				if(typec_gpoi_status==1){
						mt_set_gpio_out(__HCT_TYPEC_ACCDET_MIC_SWITCH_GPIO_NUM__ , 0);   //Set gpio num to switch mic or GND
				}else{
						mt_set_gpio_out(__HCT_TYPEC_ACCDET_MIC_SWITCH_GPIO_NUM__ , 1);
				}
				disable_accdet();
				accdet_init();	/* do set pwm_idle on in accdet_init*/
				pmic_pwrap_write(ACCDET_STATE_SWCTRL, (pmic_pwrap_read(ACCDET_STATE_SWCTRL) | ACCDET_SWCTRL_IDLE_EN));
