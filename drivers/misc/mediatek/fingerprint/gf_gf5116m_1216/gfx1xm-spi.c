...
static ssize_t gfx1xm_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	...
	if (count > GFX1XM_CFG_LEN){
		gfx1xm_error("%s:size not match [%d]",__func__,GFX1XM_CFG_LEN);
		return -EFAULT;
	}
...
//	spi_dts_gpio_init(gfx1xm_dev);
	...
	finger_irq_node = of_find_compatible_node(NULL, NULL, "mediatek,hct_finger"); //lsm modify 
	if(finger_irq_node) {
		...
		if(request_irq(gfx1xm_dev->spi->irq,gfx1xm_irq,IRQF_TRIGGER_RISING,"hct_finger",gfx1xm_dev)){
...
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
		...
		.bus_num = 0,
		.chip_select=FINGERPRINT_GF_GF5116M_1216_CS,
		.mode = SPI_MODE_0,
